...
class PutBackGPU:
    ...
    Motion sequences (optional):
      Load named image sequences via load_sequence(). When active, the background
      plate cycles through the sequence frames at native fps instead of using the
      static registration frame. Grid and mask stay cached from face registration
      (face position must be stable across sequence frames).
    """
